...
const StepNavigatorCard = ({ step, index }: { step: QuizStep, index: number }) => {
  ...
  const copyStep = () => {
    const stepToCopy = quiz.steps[index];
  ...
  const deleteStep = () => {
    quiz.steps.splice(index, 1); 
    setQuiz({ ...quiz });
  }

  return (
    <div className='flex gap-2 w-full group flex-1'>
      ...
      <FancyCard onClick={ () => { setCurrentStep(index) } } color={ index === currentStep ? 'foreground' : 'background' } className='flex-1 relative aspect-video flex justify-center items-center mr-2 md:mr-0 md:mb-4'>
